...
contract ZkEvoteHTLP is registerCommitments, Verifier {
    ...
    function setTally(tallyData calldata tData) public {
        ...
        for (uint i = 0; i < tData.D.length; i++) {
            require(
                tData.D[i] <= registeredCount,
                "tallyingResult have to be less than # of registered voters"
            );
        }
        // For simplicity, we assume that k*nCandiadates <= 256, 
        // otherwise, you have to update how 'aggregated_D' is calcluatrd using 'BigNumbers'
        ...
        tmp2 = tmp2.modexp(
            BigNumbers.init(abi.encodePacked(aggregated_D), false),
            N_square
        );
