...
assert(
  Math.floor(Math.log2(nVoters)) + 1 <= k,
  'k must be >= 1 + floor(log2(nVoters))',
);
// For simplicity, we assume k*nCandiadates <= 256, in using 'setTally' function of eVote.sol
assert(
  k*nCandidates <= 256,
  'For simplicity, we assume k*nCandidates <= 256, in using "setTally" function of eVote.sol. When  k*nCandidates > 256, you have to update "setTally" function by using bigNumber',
);
...
describe('zk-Evote-HTLP', () => {
  before(async () => {
    ...
    for (let i = 0; i < nVoters; i++) {
      voters.push(await new Voter(2 * DEPOSIT, n, n_per_word, kN, kr, lr, nCandidates, nApproves, k));
    }
