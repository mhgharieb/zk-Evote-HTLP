...
include "./templates/bigint_func.circom";
...
//k >= log_2 (n_v) + 1
template castVote(n, kN, kr, lN, lr, MerkleTreeDepth, nCandiadates, nApproves, k){
    // assert(n <= 63);
    assert(lN > nCandiadates*k );
    signal input N[kN];
    signal input g[kN];
    ...
    var kd = div_ceil(nCandiadates * k, n);
    component aggregatedVote;
    aggregatedVote = AggregatedVote(n, kd, nCandiadates, nApproves, k);
    ...
    encryptVote = EncryptVote2(n, kN, kr, kd, lN, lr);
    for(var i=0;i<kN; i++){
        encryptVote.N[i] <== N[i];
        encryptVote.g[i] <== g[i];
    }
    ...
    for(var i=0;i<kd; i++){
        encryptVote.d[i] <== aggregatedVote.d[i];
    }
    for(var i=0;i<kN; i++){
        u[i] === encryptVote.u[i];
    }
