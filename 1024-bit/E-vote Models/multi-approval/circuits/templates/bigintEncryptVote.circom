...
template EncryptVote2(n, kN, kr, kd, lN, lr){
    signal input N[kN];
    signal input g[kN];
    ...
    //2- cal (N+1)^d Mod N^2 = d*N +1 Mod N^2
    
    // k>1 and 2^{nCandiadates*k} < N by design
    // ld <= (nCandiadates-1)*k + 1
    // d < 2^{(nCandiadates-1)*k + 1}
    // d < 2^{nCandiadates*k} * 2^{-k+1}
    // d < N * 2^{-k+1}
    // N*d < N_square * 2^{-k+1} < N_square
    // N*d + 1 < N_square * 2^{-k+1} + 1 < N_square
    
    // N*d < N_square
    component N_multi_d_cal = BigMult(n, kN);
    for(var i=0; i<kN; i++){
        N_multi_d_cal.a[i] <== N[i];
    }
    for(var i=0; i<kd; i++){
        N_multi_d_cal.b[i] <== d[i];
    }
    for(var i=kd; i<kN; i++){
        N_multi_d_cal.b[i] <== 0;
    }
    // N*d + 1 < N_square
    component N_multi_d_plus_1_cal = BigAdd(n, 2*kN);
    N_multi_d_plus_1_cal.a[0] <== N_multi_d_cal.out[0];
    N_multi_d_plus_1_cal.b[0] <== 1;
    for(var i=1; i<2*kN; i++){
        N_multi_d_plus_1_cal.a[i] <== N_multi_d_cal.out[i];
        N_multi_d_plus_1_cal.b[i] <== 0;
    }
    ...
    for(var i=0; i<2*kN;i++){
        ...
        v_cal.b[i] <== N_multi_d_plus_1_cal.out[i];
