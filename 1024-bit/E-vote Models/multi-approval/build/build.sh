...
useMessage="Usage: ./build.sh -d ${GREEN}[TREE_DEPTH]${NC} -m ${GREEN}[NUMBER_of_CANDIDATES]${NC} -a ${GREEN}[NUMBER_of_APPROVAL]${NC}"
# Parsing arguments
while getopts "hd:m:a:" opt
do
   case "$opt" in
      d)
         depthOpt="$OPTARG";
...
nApproves=$nApprovesOpt
# For simplicity, set k = TREE_DEPTH + 1,
k=$((depth+1))

if [[ $nApproves -ge $nCandiadates ]]; then
   echo "${GREEN}[NUMBER_of_APPROVAL]${NC} must be less than ${GREEN}[NUMBER_of_CANDIDATES]${NC}";
   exit 1
fi
if [[ $((k*nCandiadates)) -ge "257" ]]; then
   echo "${GREEN} For simplicity, we assume that: ${NC}"
   echo "${GREEN}[NUMBER_of_CANDIDATES]${NC} * (1 + ${GREEN}[TREE_DEPTH]${NC}) must be less than 257";
   echo "Otherwise, you have to modify the smart contract 'evote.sol' such that 'setTally' function uses 'bigNumbers'"
   exit 1
fi
...
sed -i "s/__nApproves__/$nApproves/g" ../circuits/castVote_main.circom
sed -i "s/__nApproves__/$nApproves/g" ../test/2_eVote.test.js
...
For example, to test for the number of voters = 3, run
   ${GREEN}nVoters=${NC}${RED}3${NC}${GREEN} npm run test${NC}
"""
