...
useMessage="Usage: ./build.sh -d ${GREEN}[TREE_DEPTH]${NC} -m ${GREEN}[NUMBER_of_CANDIDATES]${NC} -s ${GREEN}[NUMBER_of_APPROVAL]${NC}"
# Parsing arguments
while getopts "hd:m:s:" opt
do
   case "$opt" in
      d)
         depthOpt="$OPTARG";
...
S=$SOpt
# For simplicity, set k = floor(log2(S)) + TREE_DEPTH + 1,
k=$(python3 -c "import math; print(math.floor(math.log2($S)) +$depth +1)")
...
if [[ $((k*nCandiadates)) -ge "257" ]]; then
   ...
   echo "${GREEN}[NUMBER_of_CANDIDATES]${NC} * (1 + ${GREEN}[TREE_DEPTH]${NC}+${GREEN}log2([S]${NC})) must be less than 257";
